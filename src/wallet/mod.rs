...
use bdk_chain::{
    ...
    DescriptorExt, Indexed, IndexedTxGraph, Indexer, Merge, indexed_tx_graph,
...
use bitcoin::{
    Address, Amount, Block, BlockHash, FeeRate, Network, NetworkKind, OutPoint, Psbt, ScriptBuf,
    Sequence, SignedAmount, Transaction, TxOut, Txid, Weight, Witness, absolute,
...
impl Wallet {
    ...
    pub fn apply_evicted_txs(&mut self, evicted_txs: impl IntoIterator<Item = (Txid, u64)>) {
        let canon_txids: BTreeSet<Txid> = self.canonical_view.txs().map(|c| c.txid).collect();
        let mut tx_update = TxUpdate::default();
        tx_update.evicted_ats = evicted_txs
            .into_iter()
